export default async function handler(req, res) {
  ...
  const { answers, contact, report: existingReport, emailOnly } = req.body;

  if (!answers) {
    return res.status(400).json({ error: 'No answers provided' });
  }

  // If emailOnly, skip Claude and just send emails with existing report
  if (emailOnly && existingReport && contact && contact.email) {
    try {
      ...
      const reportFormatted = existingReport.split('\n\n').map(p =>
      ...
      await fetch('https://api.resend.com/emails', {
        ...
        body: JSON.stringify({
          ...
          html: `<div style="background:#030712;padding:32px;font-family:sans-serif;max-width:700px;margin:0 auto;">
            ...
            <p style="color:#4b5563;font-size:12px;text-align:center;margin:0;">© Coseek · <a href="https://coseek.ai" style="color:#4b5563;">coseek.ai</a></p>
          </div>`
        })
      });
